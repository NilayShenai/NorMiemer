    ...
    for option in options:
        try:
            ...
            log(
                None,
                "ERROR",
                f"Unable to find configuration option for `{option}`. Make sure it is present.",
            )

    if config["auto trade"]["enabled"]:
        request = get(
            "https://discord.com/api/v10/users/@me",
            headers={"authorization": config["auto trade"]["trader token"]},
        )
        ...
        if request.status_code != 200:
            log(
                None,
                "ERROR",
                "Invalid trader token set. Please double-check you entered a valid token in `config.yml`.",
            )
        ...
        config["auto trade"]["trader"]["session_id"] = gateway(
            None, config["auto trade"]["trader token"]
        )
