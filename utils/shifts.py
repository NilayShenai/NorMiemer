    ...
    while True:
        if (
            ...
            and (
                ...
                - datetime.strptime(Client.database["shifts"]["passive"], "%x-%X")
            ).total_seconds()
            >= Client.config["shifts"]["passive"]
        ):
            ...
            and (
                datetime.strptime(datetime.now().strftime("%x-%X"), "%x-%X")
                - datetime.strptime(Client.database["shifts"]["active"], "%x-%X")
            ).total_seconds()
            >= Client.config["shifts"]["active"]
        ):
